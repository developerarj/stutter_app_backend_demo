...
def initialize_common_routes(mongo):
    ...
    def upload_audio_file():
        ...
        if file.content_type.startswith('audio'):
            ...
            if file.content_type != 'audio/mpeg':
                # Convert the audio blob to MP3
                audio_blob = AudioSegment.from_file(
                    io.BytesIO(file.read()))
                mp3_bytes = io.BytesIO()
                audio_blob.export(mp3_bytes, format="mp3")
                mp3_bytes.seek(0)
                ...
                # File is already in MP3 format, proceed as usual
    ...
    def listActivity():
        ...
        for activity in activities:
            ...
            activity['createdDate'] = str(activity['createdDate'])
            activity['updatedDate'] = str(activity['updatedDate'])
    ...
    def listActivitySession():
        ...
        activitySessions = list(mongo.db.activitySession.find(
            {'userId': str(user['_id'])}, {'_id': 1, 'activity_id': 1, 'createdDate': 1, 'updatedDate': 1}))
    ...
    def addActivitySession():
        ...
        return jsonify({
            ...
            'createdDate': new_session['createdDate'],
            'updatedDate': new_session['updatedDate']
        }), 200
    ...
    def getActivitySession(activity_id):
        ...
        activitySessions = list(mongo.db.activitySession.find(
            {'userId': str(user['_id']), 'activity_id': activity_id}, {'_id': 1, 'activity_id': 1, 'createdDate': 1, 'updatedDate': 1}))
